...
parser = OptionParser(
	usage = "Usage: %prog [options] <database> <sequences ...>")
...
parser.add_option("-j", "--threads", action="store",	dest="threads",
	default=NUM_THREADS, help="number of threads [default: %default]", 
	type="int")
...
parser.add_option("-e", "--evalue", action="store",	dest="evalue",	 
	default=MAX_EVALUE, help="maximum e-value [default: %default]", 
	type="float")
...
parser.add_option("-L", "--length",	action="store",	dest="fraction",	 
	...
	help="allowable relative error in hit length [default: %default]")
...
parser.add_option("-d", "--directory", action="store", dest="directory",	 
	default=DIRECTORY, help="minimum percent length [default: current]", 
	type="string")
parser.add_option("-P", "--plotter", action="store", dest="plotter",
	default=PLOTTER, help="plotting module [default: %default]", 
	type="string")
...
parser.add_option("--no-predict", action="store_false", dest="predicting",
	default=predicting, help="don't predict genes, " + \
		"instead treat the input files as predicted genes [default: False]")
parser.add_option("--no-cluster", action="store_false", dest="clustering",
	default=True, help="don't cluster the sequences, " + \
		"instead treat the input files as alignments [default: False]")
parser.add_option("--no-rename", action="store_false", dest="renaming",
	default=True,
	help="don't rename the fasta and clustal files [default: False]")
parser.add_option("--no-reports", action="store_false", dest="reporting",
	default=True, 
	help="don't generate files for variance data [default: False]")
...
* args'''
	...
	if '*' in opts.start:
		DNA = 'ATCG'
		opts.start = \
			set(i+j+k for i in DNA for j in DNA for k in DNA) - set(opts.stop)
