...
def run(infile, outfile, **kwargs):
	...
	if n > 1:
		...
		if seqtype == 'nucl':
			...
			others = ["-%s=%s" % (arg, kwargs.get(arg, defaults[arg])) for arg in \
				set(name.upper() for name in kwargs) & set(defaults.keys())]
			...
			others = ["-%s=%s" % (arg, kwargs.get(arg, defaults[arg])) for arg in \
				set(name.upper() for name in kwargs) & set(defaults.keys())]
			subprocess.call([cmd, "-INFILE=" + infile, "-ALIGN", "-TYPE=PROTEIN",
				"-OUTFILE=" + outfile] + others, stdout=ignore)		
		pos = infile.rfind('.')
		if pos > -1: prefix = infile[:pos]
		else: prefix = infile
